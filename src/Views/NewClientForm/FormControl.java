...
// Controlador do formulário de cadastro de novo cliente
public class FormControl extends Application {

    private Agenda agenda;
    private TableView<Client> tableView;

    // Construtor que recebe uma instância da classe Agenda e a tabela de clientes
    ...
    // Método para lidar com o evento de salvar um novo cliente
    @FXML
    private void handleSalvar(ActionEvent event) {
        ...
        // Se todos os campos forem válidos, cria um novo cliente e o adiciona à tabela
        if (!nameText.isEmpty() && !phoneText.isEmpty()
                && (!emailText.isEmpty() && emailText.matches(FormatUtils.EMAIL_REGEX))
                && (!cpfText.isEmpty() && cpfText.matches(FormatUtils.CPF_REGEX))) {
            ...
            if (isAdded == false) {
                AlertUtils.showAlert("Erro!",
                        "O cliente não foi adicionado ao sistema, pois já existe esse CPF no sistema.",
                        AlertType.WARNING);
            } else {
                tableView.getItems().add(client);
                AlertUtils.showAlert("Excelente!", "O cliente " + nameText + "foi adicionado ao sistema :D",
    ...
    // Método para formatar o CPF quando digitado
    @FXML
    ...
    // Método para formatar o número de telefone quando digitado
    @FXML
