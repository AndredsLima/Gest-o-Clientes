...
public class HomeControl extends Application {
    ...
    // Construtor que recebe uma instância da classe Agenda
    ...
    // Referências para os elementos da interface gráfica (FXML)
    ...
    // Método inicializado quando a tela é carregada
    public void initialize() {
        // Associa cada coluna da tabela a uma propriedade do cliente
        ...
        // Configura a coluna de ação para exibir um botão "Excluir"
    ...
    // Método para adicionar um novo cliente à tabela
    ...
    private void buscarPorCPF(ActionEvent event) {
        ...
        if (!cpfSearch.isEmpty()) {
            ...
            AlertUtils.showAlert("Não encontrado :(", "O CPF " + cpfSearch + " não foi encontrado no sistema.",
                    AlertType.WARNING);
        }
    }
    ...
    // Método para criar a célula da coluna de ação com um botão "Excluir"
            ...
            public TableCell<Client, SimpleStringProperty> call(final TableColumn<Client, SimpleStringProperty> param) {
                return new TableCell<Client, SimpleStringProperty>() {
                    ...
                    private void deleteClient(String clientCpf, int clientIndex) {
                        ...
                        if (hasDeleted == false) {
                            AlertUtils.showAlert("Erro :(",
                                    "O cliente com o cpf " + clientCpf + " não pôde ser deletado", AlertType.WARNING);
                        } else {
                            tableView.getItems().remove(getIndex());
                            AlertUtils.showAlert("Operação bem sucedida",
                                    "O cliente com o cpf " + clientCpf + " foi deletado do sistema.",
